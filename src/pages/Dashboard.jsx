...
const BeneficiosDoCondominio = ({ condominio, onVerDetalhes }) => {
  if (!condominio) return null

  const total = condominio.beneficios.reduce((s, b) => s + (b.valor || 0), 0)

  const handleClickDetalhes = () => {
    if (onVerDetalhes) onVerDetalhes(condominio)
  }
...
export default function Dashboard() {
  ...
  const handleVerDetalhesCondominio = (condominio) => {
    navigate(`/historico-condominio/${condominio.id}`)
  }
  ...
  return (
    <div className="dashboard-container">
      ...
      <BeneficiosDoCondominio
        condominio={selecionado}
        onVerDetalhes={handleVerDetalhesCondominio}
      />
    </div>
  )
}
